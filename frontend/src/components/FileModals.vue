<template>
  ...
  <a-modal
    ...
    title="移动到"
    @cancel="$emit('update:showMoveModal', false)"
  ...
  <a-modal
    ...
    title="创建分享"
    @cancel="$emit('update:showShareModal', false)"
  ...
  >
    <a-form :model="shareForm">
      ...
      <a-form-item label="过期时间">
        <a-date-picker
          :model-value="shareForm.expires_at"
  ...
  <a-modal :footer="false" :visible="showShareResult" title="分享成功" @cancel="$emit('update:showShareResult', false)">
    <div style="text-align: center; padding: 20px 0">
      ...
      <p style="color: var(--color-text-3); font-size: 12px; margin-top: 10px">复制链接发送给好友即可访问</p>
    </div>
  </a-modal>
</template>
